...
TcpServer::TcpServer()
    : epfd_(::epoll_create1(EPOLL_CLOEXEC)),
      ...
      socket_(ServerSocket::create_nonblock_server_fd()),
      quit_(false) {
...
void TcpServer::set_certificate(const char *cert_file,
...
void TcpServer::loop() {
  ...
  while (!quit_) {
    ...
    if (n >= (int)events_.size()) events_.resize(n * 2);
...
void TcpServer::handle_connected_conn(int connfd) {
  ...
  if (new_conn_cb_) new_conn_cb_(&conns_[connfd]);
}
